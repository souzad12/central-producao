...
public class WriterActivity extends Activity {
  ...
  private static final int TEXT_EXPORT = 2003;
  ...
  @Override public void onCreate(Bundle state) {
    ...
    web.setWebViewClient(new WebViewClient() {
      @Override public void onPageFinished(WebView view, String url) {
        ...
        String inject = "(function(){var a=document.createElement('script');a.src='file:///android_asset/ai_patch_v03.js';a.onload=function(){var w=document.createElement('script');w.src='file:///android_asset/writer_patch_v04.js';document.body.appendChild(w)};document.body.appendChild(a)})()";
  ...
  private class StoreDb extends SQLiteOpenHelper {
    StoreDb() { super(WriterActivity.this, "church_history_studio.db", null, 1); }
    ...
    @JavascriptInterface public void writeAct(String payloadJson, String model, String requestId) {
      new Thread(() -> {
        try {
          ...
          String content;
          try {
            content = DeepSeekClient.chat(requireKey(), model, WriterPrompts.system(), user, 12000, true);
            content = DeepSeekClient.normalizeJsonObject(content);
            JSONObject check = new JSONObject(content);
            if (check.optJSONArray("scenes") == null || check.optJSONArray("scenes").length() == 0) throw new Exception("JSON sem cenas escritas.");
          } catch (Exception first) {
            String retry = user + "\n\nNOVA TENTATIVA: a resposta anterior não pôde ser validada. Reescreva o ato completo em JSON estrito, conciso, mantendo a meta de palavras e todas as cenas.";
            content = DeepSeekClient.chat(requireKey(), model, WriterPrompts.system(), retry, 14000, true);
            content = DeepSeekClient.normalizeJsonObject(content);
            JSONObject check = new JSONObject(content);
            if (check.optJSONArray("scenes") == null || check.optJSONArray("scenes").length() == 0) throw new Exception("A segunda resposta também veio sem cenas escritas.");
          }
          callback("onAiWritingResult", requestId, true, content);
